# Database Schema — Yana Travels (Supabase / PostgreSQL)

**Version:** 0.1  ·  **Date:** 2026-07-21  ·  **Lens:** Database / Supabase Schema Architect

> Supabase is the **only** database, auth, and storage. Every table has RLS enabled. No data is
> imported until the PDF validation report is complete. Prices/dates/hotels stay empty
> (`NEEDS CLIENT CONFIRMATION`) until verified.

Conventions: `snake_case`, `id uuid default gen_random_uuid() primary key`, `created_at` /
`updated_at timestamptz default now()`, `created_by` / `updated_by uuid` (→ auth.users), soft
...
**Leads:** `enquiries`.
**Admin:** `admin_users` (profile+role over `auth.users`), `audit_log`.
...
| terms | text null | |
| hero_image_url | text null | Storage URL |
...
Anon may `INSERT` (rate-limited); only admins may `SELECT`/`UPDATE`.
...
### admin_users
`id (=auth.users.id) fk, full_name, role enum(super_admin,editor), is_active, created_at`.
...
| Table group | anon | authenticated admin |
|---|---|---|
...
| `enquiries` | `INSERT` only | `SELECT`/`UPDATE` |
| `admin_users`, `audit_log` | none | super_admin (admin_users), admins read audit |

Writes use server-side Supabase client; the **service-role key never reaches the browser**.
`security definer` functions only where necessary, with fixed `search_path`.

---

## 5. Storage buckets
- `logos` (public read), `packages` (public read; images/pdf), `gallery` (public read),
  `brochures` (public read), `hero` (public read). Uploads authenticated-only via signed policies;
  size/type limits enforced in app + bucket policy.
...
- Migrations authored via Supabase migration tooling; **real project-ref + token required**
  (currently placeholders in `.mcp.json`).
