...
export default async function HomePage() {
  const { items: featured, sample } = await listPackages({ featured: true });
              ...
              <div className="lg:col-span-7">
                <div className="max-w-3xl p-1 sm:p-2">
                  <h1 className="max-w-3xl animate-rise font-display text-4xl font-semibold leading-[0.96] tracking-[-0.045em] text-white drop-shadow-[0_18px_34px_rgba(0,0,0,0.58)] sm:text-5xl md:text-[3.7rem] xl:text-[4.35rem]">
                    ...
                    <span className="block text-[#f6d77f]">then turns you into a story teller.</span>
                  </h1>
        ...
        <div className="relative z-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {categories.map(({ href, icon: Icon, title, desc }, i) => (
            <Reveal key={href} delay={i * 90}>
              ...
              >
                <span className="absolute inset-x-6 top-0 h-1 rounded-b-full bg-[linear-gradient(90deg,#d7a92e,#f6d77f,#d7a92e)]" aria-hidden />
                ...
                <h3 className="relative mt-5 text-lg font-bold text-deep">{title}</h3>
                <p className="relative mt-2 text-sm leading-6 text-text-secondary">{desc}</p>
              </Link>
            </Reveal>
          ))}
        </div>
            ...
            <Reveal key={c.title} delay={i * 110}>
              ...
              >
                <Image
                  ...
                  className="object-cover brightness-[1.08] contrast-[1.05] saturate-[1.16] transition-transform duration-[1200ms] ease-out group-hover:scale-105"
                />
                <div className="absolute inset-x-0 bottom-0 bg-white/94 p-5 backdrop-blur-sm">
                  <h3 className="text-xl font-bold leading-tight text-deep">{c.title}</h3>
                  <p className="mt-1.5 text-sm font-normal text-text-secondary">{c.blurb}</p>
                </div>
              </Link>
            </Reveal>
          ))}
            ...
            <div className="relative overflow-hidden rounded-[1.9rem] shadow-[0_28px_60px_-32px_rgba(16,33,58,0.38)]">
              <div className="relative aspect-[4/3]">
                <Image
                  ...
                  className="object-cover brightness-[1.08] contrast-[1.05] saturate-[1.14]"
                />
              </div>
              <div className="border-t border-[#eadfcf] bg-white p-5">
                ...
                <p className="mt-2 text-sm leading-6 text-text-secondary">
                  Enquiry, planning, documentation, and departure support in one smoother journey.
                </p>
              </div>
            </div>
          </Reveal>
          <Reveal delay={120}>
            <div>
              ...
              <p className="mt-4 text-base leading-8 text-text-secondary">
                From flights and stays to sightseeing and documentation, the site now frames the same travel offer with better spacing, more confidence, and stronger luxury cues.
              </p>
              <ul className="mt-6 space-y-3">
                {[
                  ...
                  <li key={line} className="flex items-start gap-3 font-medium text-text-main">
