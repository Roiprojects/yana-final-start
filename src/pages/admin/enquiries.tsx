...
import type { EnquiryRow } from "@/lib/types/admin";
...
export function AdminEnquiriesPage() {
  ...
  async function load() {
    try {
      const res = await api.adminEnquiries();
      setItems((res.items as EnquiryRow[]) ?? []);
      setError(null);
    } catch (e) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-text-secondary">
          {items.length} enquir{items.length === 1 ? "y" : "ies"} from the
          website.
        </p>
      </div>

      {error ? (
        <p className="text-sm text-danger">Couldn't load enquiries: {error}</p>
      ) : loading ? (
