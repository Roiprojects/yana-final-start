...
test("create → appears public → edit → delete", async ({ page }) => {
  ...
  await page.getByLabel("Highlights (one per line)").fill("Beaches\nSunset cruise");
  await page.getByRole("button", { name: /create package/i }).click();
  await expect(page).toHaveURL(/\/admin\/packages$/, { timeout: 15000 });
  await expect(page.getByText("E2E Test Goa Getaway")).toBeVisible({ timeout: 20000 });

  // Appears on public detail page
  await page.goto(`/packages/${SLUG}`);
  await expect(page.getByRole("heading", { name: "E2E Test Goa Getaway" })).toBeVisible({ timeout: 20000 });
  await expect(page.getByText(/₹18,999/).first()).toBeVisible({ timeout: 10000 });

  // Edit
  await page.goto("/admin/packages");
  await page.getByRole("row", { name: /E2E Test Goa Getaway/ }).getByRole("link", { name: /edit/i }).click();
  ...
  // Delete
  await page.getByRole("row", { name: /E2E Test Goa Updated/ }).getByRole("link", { name: /edit/i }).click();
  await expect(page).toHaveURL(/\/edit$/, { timeout: 15000 });
